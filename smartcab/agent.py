...
class LearningAgent(Agent):
    ...
    def reset(self, destination=None, testing=False):
        ...
        #self.epsilon = (1-epsilon_decay_rate)*self.epsilon
        a = 0.997
        self.t +=1
    ...
    def choose_action(self, state):
        ...
        if state not in self.Q.keys():
            q = {'forward':0, 'right':0, 'left':0, None: 0}
        else:    
            q= self.Q[state]
        if self.learning == False:
            action = random.choice(q.keys())
            ...
            if self.epsilon > random.random():
               action = random.choice(q.keys()) 
                ...
                q= dict((k, v) for k, v in q.items() if v == maxq)
                action = random.choice(q.keys())
        return action
    ...
    def learn(self, state, action, reward):
        ...
        if self.learning == True:
            Q_learn=self.Q
            q=Q_learn[state]
            q[action]= (1-self.alpha)*q[action] + (reward*self.alpha)
            self.Q = Q_learn
        return
...
def run():
    ...
    # Flags:
    #update_delay = 0.1 #continuous time (in seconds) between actions, default is 2.0 seconds
    #   display      - set to False to disable the GUI if PyGame is enabled
    # log_metrics = True  # - set to True to log trial and simulation results to /logs
    #   optimized    - set to True to change the default log file name
    sim = Simulator(env, log_metrics = True, update_delay = 0.001, optimized = True, display = False)
